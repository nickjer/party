  ...
  module Broadcast
    class PlayerDisconnectedTest < ActiveSupport::TestCase
      ...
      test "#call broadcasts to other online players" do
        ...
        # Alice disconnecting should broadcast to Bob
        assert_turbo_stream_broadcasts bob.to_model, count: 1 do
          PlayerDisconnected.new(player_id: alice.id).call
        end
      end

      test "#call does not broadcast to disconnected player" do
        game = create(:bu_polling_game, player_names: %w[Alice Bob])
        alice = game.players.find { |p| p.name.to_s == "Alice" }
        bob = game.players.find { |p| p.name.to_s == "Bob" }
        ...
        # Alice disconnecting should not broadcast to Alice herself
      ...
      test "#call does not broadcast to offline players" do
        ...
        # Alice disconnecting should not broadcast to offline Bob
        assert_turbo_stream_broadcasts bob.to_model, count: 0 do
          PlayerDisconnected.new(player_id: alice.id).call
        end
      end

      test "#call broadcasts to multiple online players except disconnected " \
        "player" do
        ...
        # Alice disconnecting should broadcast to Bob and Charlie, but not Alice
        assert_turbo_stream_broadcasts bob.to_model, count: 1 do
      ...
      test "#call handles game with only disconnected player" do
        game = create(:bu_game, :with_judge) # Only judge
