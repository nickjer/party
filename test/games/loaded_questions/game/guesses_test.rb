    ...
    class GuessesTest < ActiveSupport::TestCase
      test "#correct? returns true when case-insensitive answers match despite different player ids" do
        ...
        if guess1.player.id == guess1.guessed_player.id
          game.swap_guesses(player_id_1: guess1.player.id, player_id_2: guess2.player.id)
          game = LoadedQuestions::Game.find(game.slug)
          guess1, guess2 = game.guesses.to_a
        end

        # Assert player IDs don't match guessed player IDs
        assert_not_equal guess1.player.id, guess1.guessed_player.id, "First guess should have different player and guessed_player IDs"
        assert_not_equal guess2.player.id, guess2.guessed_player.id, "Second guess should have different player and guessed_player IDs"

        # Assert both are correct due to case-insensitive matching
        assert_predicate guess1, :correct?, "First guess should be correct due to case-insensitive match"
        assert_predicate guess2, :correct?, "Second guess should be correct due to case-insensitive match"
